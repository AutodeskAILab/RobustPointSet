...
For this strategy, we train on `train_original.npy` without applying any data-augmentation, select the best performing model on `test.original` (to be consistent with baselines), and test on each test set (i.e. `test_*.npy` ) separately.
